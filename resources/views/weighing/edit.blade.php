...
@section('title', 'Edit Weighing Transaction')
...
<div class="px-4 sm:px-0">
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">Edit Weighing Transaction</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <form action="{{ route('weighing.update', $weighing->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="supplier_id" id="supplier_id" required
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" {{ (old('supplier_id', $weighing->supplier_id) == $supplier->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="vehicle_id" id="vehicle_id" required
                        ...
                        @foreach($vehicles as $vehicle)
                            <option value="{{ $vehicle->id }}" {{ (old('vehicle_id', $weighing->vehicle_id) == $vehicle->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="gross_weight" id="gross_weight" 
                           value="{{ old('gross_weight', $weighing->gross_weight) }}" required
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="tare_weight" id="tare_weight" 
                           value="{{ old('tare_weight', $weighing->tare_weight) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="net_weight_display" readonly
                           value="{{ number_format($weighing->net_weight, 2) }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="weighing_time" id="weighing_time" 
                           value="{{ old('weighing_time', $weighing->weighing_time->format('Y-m-d\TH:i')) }}" required
            ...
            @if($weighing->vehicle_photo)
                <div>
                    ...
                    <img src="{{ asset('storage/' . $weighing->vehicle_photo) }}" alt="Vehicle Photo" class="max-w-xs rounded-lg shadow">
                </div>
            @endif
            ...
            <div>
                <label for="vehicle_photo" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Vehicle Photo {{ $weighing->vehicle_photo ? '(Upload new to replace)' : '' }}
                </label>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('notes') border-red-500 @enderror">{{ old('notes', $weighing->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
